...
\begin{table}[H]
    ...
    \begin{tabular}{ccc}
        ...
        60 & 95.96\% & 98.28\% \\
        70 & 97.32\% & 98.44\% \\
        79 & 97.92\% & \textbf{98.56\%}（早停） \\
        \bottomrule
    \end{tabular}
\end{table}

图~\ref{fig:confusion} 展示了 SE-ResNet34 在验证集上的混淆矩阵。
...
从混淆矩阵可以观察到：在验证集 2,500 张图片中，猫类正确预测 1,246 张（误判为狗 17 张），狗类正确预测 1,220 张（误判为猫 17 张），仅 34 张图片分类错误。两类的误判数量完全对称（均为 17 张），说明模型不存在系统性偏向。错误样本可能属于以下情况：（1）猫狗外形高度相似的品种（如部分长毛猫与小型犬）；（2）图片中动物占比极小或被遮挡；（3）非典型姿态或极端光照条件。
...
从混淆矩阵的分析中，验证集共 34 张图片分类错误（猫误判为狗 17 张，狗误判为猫 17 张）。对错误样本的分析揭示了以下典型失败模式：
...
上述错误模式均为二分类任务中的固有难点，在 98.56\% 的验证精度下，剩余的 1.44\% 错误主要集中在这些边界样本上。通过进一步增大训练数据规模、引入更精细的数据增强策略（如针对遮挡的 CutOut），有望进一步减少此类错误。

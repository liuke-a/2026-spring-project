...
\begin{table}[H]
    ...
    \begin{tabular}{lcc}
        ...
        \midrule
        Accuracy  & 97.76\% & \textbf{98.56\%} \\
        Precision & 98.13\% & \textbf{98.63\%} \\
        Recall    & 97.33\% & \textbf{98.47\%} \\
        F1-Score  & 97.73\% & \textbf{98.55\%} \\
        \midrule
        参数量 & 11.7M & 21.3M \\
...
图~\ref{fig:confusion} 展示了 SE-ResNet18 在验证集上的混淆矩阵。
...
从混淆矩阵可以观察到：在验证集 2,500 张图片中，猫类正确预测 1,240 张（误判为狗 23 张），狗类正确预测 1,204 张（误判为猫 33 张），共 56 张图片分类错误。狗类误判为猫的数量（33 张）略多于猫类误判为狗的数量（23 张），可能是因为部分长毛品种的猫在外形上与小型犬更为相似。错误样本可能属于以下情况：（1）猫狗外形高度相似的品种（如部分长毛猫与小型犬）；（2）图片中动物占比极小或被遮挡；（3）非典型姿态或极端光照条件。
...
\begin{enumerate}
    ...
    \item \textbf{网络深度的影响}：SE-ResNet34 相比 SE-ResNet18 在 Accuracy 上提升了约 0.80\%（97.76\% $\rightarrow$ 98.56\%），额外参数量约 82\%（11.7M $\rightarrow$ 21.3M），GPU 显存占用从 4.4GB 增至 6.1GB。更深的网络能够提取更丰富的层次化特征，从而提升分类精度。
...
\begin{itemize}
    ...
    \item \textbf{从头训练的可行性}：在 25,000 张图片的数据集上，不依赖预训练权重的 SE-ResNet 从头训练即可达到 97.76\%--98.56\% 的验证精度，说明自定义架构在中等规模数据集上具有足够的学习能力。
...
上述错误模式均为二分类任务中的固有难点，在 97.76\% 的验证精度下，剩余的 2.24\% 错误主要集中在这些边界样本上。通过进一步增大训练数据规模、引入更精细的数据增强策略（如针对遮挡的 CutOut），或使用更深的网络结构（如 SE-ResNet34），有望进一步减少此类错误。
...
本项目设计并实现了一套基于自定义 SE-ResNet 的猫狗图片分类系统。通过将 Squeeze-and-Excitation 通道注意力机制嵌入 ResNet 残差块，结合 Mixup/CutMix、RandAugment、DropPath、Label Smoothing 等多种正则化策略，以及混合精度训练、EMA 权重平滑、早停等工程化训练技术，在不依赖预训练权重的条件下，SE-ResNet18 验证集精度达到 97.76\%，SE-ResNet34 达到 98.56\%。
